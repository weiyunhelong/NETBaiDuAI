...
var uploadify_onSelectError = function (file, errorCode, errorMsg) {
    ...
    switch (errorCode) {
        case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
            ...
            {
                msgText += "最多可以上传 " + this.settings.uploadLimit + "个文件";
            }
            else
            {
...
function chkAll() {
    ...
    if ($("#checkAll").prop("checked")) {
        ...
        $(":checkbox").parent().parent().addClass("trSelectClass");
    } else {
        $(":checkbox").prop("checked", false);
        $(":checkbox").parent().parent().removeClass("trSelectClass");
    }
}

//验证权限
function ValidationAuthorize(url, isShowTip) {
    var result = false;
    GetAjaxPost("/Main/ValidationAuthorize?actionUrl=" + url + "", "html", function (data) {
        if (data == "True") {
            result = true;
...
function layerOpen(title, content, width, height) {
    top.layer.open({
        type: 2,
        title: [title, "background:#539214;color:#fff;text-align:left;font-size:16px;margin: 0px;padding-left: 10px;"],
