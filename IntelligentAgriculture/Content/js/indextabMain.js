...
(function ($) {
    $.nfinetab = {
        ...
        refreshTabById: function (id) {
            var target = $('.NFine_iframe[data-id="' + id + '"]');
            var url = target.attr('src');
            $.loading(true);
            target.attr('src', url).load(function () {
                $.loading(false);
        ...
        addTab: function () {
            ...
            if (Atype == "menu") {
                $(".divTitleA ").each(function () {
                    $(this).removeClass("divTitleASelct");
                })
                $("#divTopOne").addClass("divTitleASelct");
            }
            $(".menuItem").each(function () {
                var itemDataId = $(this).attr("data-id");
                if (dataid == itemDataId) {
                    $(this).css("background", "#F3F7FD");
                    $(this).css("color", "#7CAE4A");
                    $("#img_" + itemDataId).attr("src", "/Content/Images/首页/erjicaidan_selected.png");
                }
                else if (Atype == "menu") {
                    $(this).css("background", "#7CAE4A");
                    $(this).css("color", "#fff");
                    $("#img_" + itemDataId).attr("src", "/Content/Images/首页/erjicaidan.png");
                }
            })
            $(".divTemplate").each(function () {
                $(this).removeClass("divTemplateSelect");
            })
            $("#divData_" + regionid).addClass("divTemplateSelect");
            ...
            var dataUrl = $(this).attr('href') + "?WS_GreenhouseId=" + dataId;
            ...
            if (flag) {
                var height = document.body.clientHeight - 167;
                var str = '<a href="javascript:void();" class="active menuTab" data-id="' + dataUrl + '">' + menuName + ' <i class="fa fa-remove"></i></a>';
                $('.menuTab').removeClass('active');
                var str1 = '<iframe style="padding: 0px 30px; " class="NFine_iframe" id="iframe' + dataId + '" name="iframe' + dataId + '"  width="100%" height="' + height + '" src="' + dataUrl + '" frameborder="0" data-id="' + dataUrl + '" seamless></iframe>';
                $('.mainContent').find('iframe.NFine_iframe').hide();
                $("#firstIframe").show();
                $("#firstIframe").attr("src", dataUrl);
        ...
        init: function () {
            $('.dropdown-toggle').on('click', $.nfinetab.addTab);
